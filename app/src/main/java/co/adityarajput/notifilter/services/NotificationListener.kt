...
class NotificationListener : NotificationListenerService() {
    ...
    override fun onListenerConnected() { super.onListenerConnected(); serviceScope.launch { runCatching { TextToSpeech.initialize(this@NotificationListener) }.onFailure { Logger.e("TextToSpeech", "Failed to initialize TTS", it) } }; requestListenerHints(0); serviceScope.launch { delay(500.milliseconds); reconcilePendingNotifications() } }
    ...
    override fun onNotificationPosted(sbn: StatusBarNotification) {
        ...
        when (filter.action) {
            ...
            is Action.READ -> { if (TextToSpeech.ready) TextToSpeech.speak(filter.action.speechTemplate.replaceWithNotificationData(notification, Cache.getAllPackages(packageManager)), sbn.id) else serviceScope.launch { runCatching { TextToSpeech.initialize(this@NotificationListener); if (TextToSpeech.available) TextToSpeech.speak(filter.action.speechTemplate.replaceWithNotificationData(notification, Cache.getAllPackages(packageManager)), sbn.id) }.onFailure { Logger.e("TextToSpeech", "Failed to read notification", it) } } }
        }
