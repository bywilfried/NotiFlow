...
class NotificationListener : NotificationListenerService() {
    ...
    override fun onNotificationPosted(sbn: StatusBarNotification) {
        ...
        if (filter == null) { if (wasPending) removePending(sbn.key); return }
        when (filter.action) {
            is Action.DISMISS -> { if (wasPending) removePending(sbn.key); dismissNotification(sbn.key, sbn.isClearable) }
            is Action.TAP_NOTIFICATION -> { if (wasPending) removePending(sbn.key); try { intents.launchMain() } catch (_: Exception) { return } }
            is Action.TAP_BUTTON -> { if (wasPending) removePending(sbn.key); try { intents.actions.entries.find { filter.action.buttonRegex.containsMatchIn(it.key) }?.value?.send() } catch (_: Exception) { return } }
            is Action.BATCH -> { val zone = ZoneId.systemDefault(); val now = ZonedDateTime.now(zone); val today = now.toLocalDate().atStartOfDay(zone); var batchLength = filter.action.batchLength * 3600000L; if (applicationInfo.flags and ApplicationInfo.FLAG_DEBUGGABLE != 0) batchLength /= 60; val until = batchLength - today.until(now, MILLIS) % batchLength; if (until < 1000L || batchLength - until < 1000L) { if (wasPending) removePending(sbn.key) } else if (notifications.any { it.data == notification.data }) { if (wasPending) removePending(sbn.key); return } else snoozePending(sbn, filter, min(until, now.until(today.plusDays(1), MILLIS))) }
            is Action.DELAY -> { val zone = ZoneId.systemDefault(); val now = ZonedDateTime.now(zone); val delay = if (filter.action.delayLength != null) filter.action.delayLength * 60000L else { val end = filter.schedule.endOfActiveRange(now) ?: run { if (wasPending) removePending(sbn.key); return }; now.until(end, MILLIS) }; if (delay < 1000L) { if (wasPending) removePending(sbn.key) } else if (notifications.any { notification.matches(it, delay) }) { if (wasPending) removePending(sbn.key); return } else snoozePending(sbn, filter, delay) }
            is Action.DEBOUNCE -> { if (wasPending) removePending(sbn.key); if (!cooldowns.containsKey(filter.id)) { cooldowns += filter.id to System.currentTimeMillis() + filter.action.cooldownLength * 60000L; muteNotificationsWhileCooldown(filter) } else cooldowns += filter.id to System.currentTimeMillis() + filter.action.cooldownLength * 60000L }
            is Action.MUTE -> { if (wasPending) removePending(sbn.key); serviceScope.launch { delay(300.milliseconds); requestListenerHints(HINT_HOST_DISABLE_NOTIFICATION_EFFECTS); delay(NOTIFICATION_SOUND_DURATION.milliseconds); requestListenerHints(0) } }
            is Action.ALERT -> { if (wasPending) removePending(sbn.key); notificationManager.notify(Constants.ALERT_NOTIFICATION_ID, NotificationCompat.Builder(this, Constants.ALERT_NOTIFICATION_CHANNEL_ID).setContentTitle(getString(R.string.app_name_launcher)).setSmallIcon(R.drawable.ic_launcher_foreground).setAutoCancel(true).build()) }
            is Action.DISTURB -> { if (wasPending) removePending(sbn.key); if (!cooldowns.containsKey(filter.id) && audioManager.ringerMode != AudioManager.RINGER_MODE_NORMAL) { cooldowns += filter.id to System.currentTimeMillis() + filter.action.pauseLength * 60000L; snoozeNotification(sbn.key, 100); disableDNDWhileCooldown(filter) } }
            is Action.DISMISS_STALE -> { if (wasPending) removePending(sbn.key); serviceScope.launch { var retention = filter.action.retentionLength * 60000L; if (applicationInfo.flags and ApplicationInfo.FLAG_DEBUGGABLE != 0) retention /= 5; alarmManager.sendIntent(this@NotificationListener, retention, sbn.key.hashCode()) { action = Constants.ACTION_DISMISS_STALE; putExtra(Constants.EXTRA_SBN_KEY, sbn.key); putExtra(Constants.EXTRA_SBN_IS_CLEARABLE, sbn.isClearable) } } }
            is Action.REPLACE -> { if (wasPending) removePending(sbn.key); createReplaceNotificationChannel(filter.id); val packages = Cache.getAllPackages(packageManager); cancelNotification(sbn.key); notificationManager.notify(Constants.getReplaceNotificationId(filter.id), NotificationCompat.Builder(this, Constants.getReplaceNotificationChannelId(filter.id)).setContentTitle(filter.action.titleTemplate.replaceWithNotificationData(notification, packages)).setContentText(filter.action.contentTemplate.replaceWithNotificationData(notification, packages)).setSmallIcon(R.drawable.ic_launcher_foreground).setContentIntent(sbn.notification.contentIntent).build()) }
        }
